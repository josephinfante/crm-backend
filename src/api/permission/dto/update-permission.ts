...
import { Permission, Role } from "../../../shared/schemas";
...
export async function UpdatePermission(id: string, data: PermissionUpdateInterface){
    try {
        ...
        const component = data.component ? await Role.findOne({ where: { name: data.component } }).catch(_error => { throw new ComponentError('Ha ocurrido un error al encontrar el componente.') }).then(component => component) : null;
        permission.set({
            create: data.create,
            read: data.read,
            update: data.update,
            delete: data.delete,
            role_id: data.role && (role?.dataValues.id !== permission.dataValues.role_id) ? role?.dataValues.id : permission.dataValues.role_id,
            component_id: data.component && (component?.dataValues.id !== permission.dataValues.component_id) ? component?.dataValues.id : permission.dataValues.component_id,
            updatedAt: new Date(),
        });
