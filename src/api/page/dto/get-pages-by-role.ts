...
export async function GetPagesByRole(name: string) {
    try {
        ...
        const pages = await Page.findAll({where: {role_id: role.dataValues.id}}).catch(_error => {throw new PageError('Ha ocurrido un error al revisar las páginas del rol')}).then(pages => pages);
