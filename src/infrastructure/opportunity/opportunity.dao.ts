...
import { BusinessUnitError, CampusError, CareerError, ContactChannelError, ContactError, OpportunityError, PeriodError, SalePhaseError } from "../../shared/errors";
import { BusinessUnitModel, CampusModel, CareerModel, ContactChannelModel, ContactModel, OpportunityModel, PeriodModel, SalePhaseModel } from "../../shared/models";
...
class OpportunityDao {
    async create(access: IAccessPermission, opportunity: Opportunity): Promise<IOpportunityResponse> {
        try {
            ...
            return OpportunityPresenter(new_opportunity, access, contact?.dataValues, career?.dataValues, period?.dataValues, campus?.dataValues, business_unit?.dataValues, sale_phase?.dataValues, contact_channel?.dataValues);
        } catch (error) {
    ...
    async update(access: IAccessPermission, id: string, opportunity: Opportunity): Promise<IOpportunityResponse> {
        try {
            ...
            opportunity_exist.set({
                ...
                competitor: opportunity.competitor ?? opportunity_exist.dataValues.competitor,
                ...
                updatedAt: Date.now(),
                contact_id: opportunity.contact_id !== opportunity_exist.dataValues.contact?.dataValues.id ? opportunity.contact_id : opportunity_exist.dataValues.contact?.dataValues.id,
                career_id: opportunity.career_id !== opportunity_exist.dataValues.career?.dataValues.id ? opportunity.career_id : opportunity_exist.dataValues.career?.dataValues.id,
                period_id: opportunity.period_id !== opportunity_exist.dataValues.period?.dataValues.id ? opportunity.period_id : opportunity_exist.dataValues.period?.dataValues.id,
                campus_id: opportunity.campus_id !== opportunity_exist.dataValues.campus?.dataValues.id ? opportunity.campus_id : opportunity_exist.dataValues.campus?.dataValues.id,
                business_unit_id: opportunity.business_unit_id !== opportunity_exist.dataValues.business_unit?.dataValues.id ? opportunity.business_unit_id : opportunity_exist.dataValues.business_unit?.dataValues.id,
                sale_phase_id: opportunity.sale_phase_id !== opportunity_exist.dataValues.sale_phase?.dataValues.id ? opportunity.sale_phase_id : opportunity_exist.dataValues.sale_phase?.dataValues.id,
                contact_channel_id: opportunity.contact_channel_id !== opportunity_exist.dataValues.contact_channel?.dataValues.id ? opportunity.contact_channel_id : opportunity_exist.dataValues.contact_channel?.dataValues.id,
            });
            ...
            return OpportunityPresenter(updated.dataValues, access, contact?.dataValues, career?.dataValues, period?.dataValues, campus?.dataValues, business_unit?.dataValues, sale_phase?.dataValues, contact_channel?.dataValues);
        } catch (error) {
            if (error instanceof Error && error.message) throw new OpportunityError(error.message);
    ...
    async findById(access: IAccessPermission, id: string): Promise<IOpportunityResponse> {
        try {
            ...
            return OpportunityPresenter(opportunity_exist.dataValues, access, opportunity_exist.dataValues.contact?.dataValues, opportunity_exist.dataValues.career?.dataValues, opportunity_exist.dataValues.period?.dataValues, opportunity_exist.dataValues.campus?.dataValues, opportunity_exist.dataValues.business_unit?.dataValues, opportunity_exist.dataValues.sale_phase?.dataValues, opportunity_exist.dataValues.contact_channel?.dataValues);
        } catch (error) {
            if (error instanceof Error && error.message) throw new OpportunityError(error.message);
    ...
    async findAll(access: IAccessPermission, page: number): Promise<{ opportunities: {}[], total_opportunity: number, total_pages: number, current_page: number}> {
        try {
            ...
            return {
                opportunities: opportunities.rows.map(opportunity => OpportunityPresenter(opportunity.dataValues, access, opportunity.dataValues.contact?.dataValues, opportunity.dataValues.career?.dataValues, opportunity.dataValues.period?.dataValues, opportunity.dataValues.campus?.dataValues, opportunity.dataValues.business_unit?.dataValues, opportunity.dataValues.sale_phase?.dataValues, opportunity.dataValues.contact_channel?.dataValues)),
